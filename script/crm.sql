...
-- procedimiento almacenado que ejecuta el procedimiento calcularTVP mediante uso de cursor sobre la tabla cotizaciones
create procedure calculoInflacion
	@idCotizacion SMALLINT,
	@contacto SMALLINT,
	@oportunidad varchar(15),
	@fechaCotizacion smallint,
...
as
begin try
	DECLARE cotizacionInfo CURSOR FOR SELECT 
									  numeroCotizacion, nombreOportunidad, contactoAsociado, fechaCotizacion, nombreCuenta
									  FROM cotizaciones

	OPEN cotizacionInfo
	FETCH NEXT FROM cotizacionInfo INTO @idCotizacion, @oportunidad, @contacto, @fechaCotizacion, @cuenta
	WHILE @@fetch_status = 0
	BEGIN
		EXEC calcularTVP @idCotizacion,
						@contacto,
						@oportunidad,
						@fechaCotizacion,
						@cuenta,
	    ...
	    FETCH NEXT FROM cotizacionInfo INTO @idCotizacion, @oportunidad, @contacto, @fechaCotizacion, @cuenta
	END
	CLOSE cotizacionInfo 
	DEALLOCATE cotizacionInfo 
